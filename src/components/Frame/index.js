...
const Frame = ({ title, footer, children, showExpand, showClose, showMinimize, className }) => {
  const [status, setStatus] = useState(STATUSES.default);
  const modalRef = useRef();
  const [layoutId,] = useState(uuidv4());
  const inExpanded = status === STATUSES.expanding || status === STATUSES.expanded || status === STATUSES.closing;
  const zIndex = inExpanded ? EXPANDED_Z_INDEX : DEFAULT_Z_INDEX;

  useEffect(() => {
    if (status === STATUSES.expanding) {
      const delay = (FrameExpandedVariant.animate.transition.duration * 1000) + (FrameExpandedVariant.animate.transition.delay * 1000);
      setTimeout(() => {
  ...
  const onMinimizeToggle = () => {
    setStatus(prevState => prevState === STATUSES.minimized ? STATUSES.default : STATUSES.minimized);
  }
